...
$step = $i+1;
if ($_GET['i'] == $step) {
$template["NAVIGATION"][] = "<a class='active' href='".$PHP_SELF."?page=".$page_number."&i=".$step."'> ".($i+1). " </a>";
 } else {
$template["NAVIGATION"][] = "<a href='".$PHP_SELF."?page=".$page_number."&i=".$step."'> ".($i+1). " </a>";
}
...
$template["NAVIGATION"][] = "<a class='active' href='".$PHP_SELF."?page=".$page_number."'> ".($i+1)." </a>";
} else {
$template["NAVIGATION"][] = "<a href='".$PHP_SELF."?page=".$page_number."'> ".($i+1)." </a>";
}
...
`firma`,
`shows`
 FROM `tovar` WHERE `kolvo`>0 GROUP BY `article` LIMIT $b,$articles_per_page"); // список товара в открытой категории
...
include $template["TEMPLATE_PATH"].'index.php'; // подключение страницы шаблона
?>