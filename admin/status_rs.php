...
if ($user_role=='1') {
    $sql_get_device = $pdo->getRows("SELECT * FROM `status_rs` ORDER BY `name` DESC ");
    foreach ( $sql_get_device as $data_get_device ) { ?>
                  <tr>
                    <td><?php echo $data_get_device['name']; ?></td>
                    <td><?php echo $data_get_device['komment']; ?></td>
                    <td><a href="classes/App.php?action=del_status_rs&id=<?php echo $data_get_device['id']; ?>"><font color="red">Удалить</font></a>
...
<?php }} ?>
                    ...
                    <input type="hidden" name="action" value="add_status_rs">
                    </div>
                    </div>
